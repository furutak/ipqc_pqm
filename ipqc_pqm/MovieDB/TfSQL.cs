...
namespace IpqcDB
{
    public class TfSQL
    {
        ...
        // 測定結果の一括登録
        public bool sqlMultipleInsert(string model, string process, string inspect, DateTime lot, DateTime inspectdate, string line, DataTable dt)
        {
            int res1;
            bool res2 = false;
            connection = new NpgsqlConnection(conStringIpqcDbP4);
            ...
            try
            {
                ...
                // 登録②：
                string sql2 = "insert into tbl_measure_history(model, process, inspect, lot, inspectdate, line, qc_user, row_set, m1, m2, m3, m4, m5, x, r) " +
                    "VALUES (:model, :process, :inspect, :lot, :inspectdate, :line, :qc_user, :row_set, :m1, :m2, :m3, :m4, :m5, :x, :r)";
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    command2.Parameters[7].Value = k.ToString();
                    command2.Parameters[8].Value = dt.Rows[i]["m1"];
                    command2.Parameters[9].Value = dt.Rows[i]["m2"];
                    command2.Parameters[10].Value = dt.Rows[i]["m3"];
                    command2.Parameters[11].Value = dt.Rows[i]["m4"];
                    command2.Parameters[12].Value = dt.Rows[i]["m5"];
                    command2.Parameters[13].Value = dt.Rows[i]["x"];
                    command2.Parameters[14].Value = dt.Rows[i]["r"];
